...
static void write_html_report(const vector<CandidateResult>& greedy_list,
    ...
    .grid { display:grid; gap:28px; grid-template-columns:repeat(auto-fit,minmax(420px,1fr)); }
    .col h2 { margin:0 0 10px; font-size:20px; font-weight:500; }
    ...
    // Compute best total across BOTH greedy and beam lists
    double best_total = 0.0;
    for(const auto& c : greedy_list) best_total = std::max(best_total, c.sim_result.total_coverage*100.0);
    for(const auto& c : beam_list)   best_total = std::max(best_total, c.sim_result.total_coverage*100.0);
    ofs << "<div class=metric><div class=lbl>Best Total%</div><div class=val>"<< std::fixed << std::setprecision(2) << best_total <<"%</div></div>";
    ofs << "</div>"; // meta

    ofs << "<div class=grid>";
    ofs << "<div class=col section><h2>Greedy Prefixes (Top "<< greedy_list.size() << ")</h2>";
    // Longest prefix first: reverse order
    for(std::size_t i=greedy_list.size(); i>0; --i){ ofs << render_candidate_card(greedy_list[i-1], w_state, w_total, op_penalty); }
    ofs << "</div>";
    ofs << "<div class=col section><h2>Beam Finals (Top "<< beam_list.size() << ")</h2>";
    for(const auto& cr: beam_list) ofs << render_candidate_card(cr, w_state, w_total, op_penalty);
    ofs << "</div>";
    ofs << "</div>"; // grid

    ofs << "<div class=footer>Generated by Template Search Engine &middot; Coverage values shown as percentages.</div>";
    ofs << "</body></html>";
}
...
int run_template_search_html(const string& faults_json_path,
    ...
    const size_t TOPK_BEAM = beam_width; // top beam results reflect beam width
    // Greedy: collect all prefixes (one per level) so top_k = skeleton_length
    auto t0 = std::chrono::steady_clock::now();
    auto greedy_list = run_greedy_prefixes(sim, lib, faults, tps, skeleton_length, skeleton_length, scorer);
    ...
    write_html_report(greedy_list, beam_list, output_html_path, w_state, w_total, op_penalty, slot_count, greedy_ms, beam_ms, TOPK_BEAM);
    write_all_results_json(greedy_list, beam_list, json_out_path.empty()?output_html_path:json_out_path);
    return 0;
}
...
int main(int argc, char** argv){
    ...
    seq_constraints.add(std::make_shared<FirstElementHasWriteNoReadConstraint>());
    ...
    // Time greedy (optional) to compare
    auto t0 = std::chrono::steady_clock::now();
    auto greedy_list = run_greedy_prefixes(sim, lib, faults_vec, tps_vec, L, L, scorer);
    auto t1 = std::chrono::steady_clock::now();
    auto greedy_ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
    std::cout << "[Greedy] Elapsed: "<< greedy_ms << " ms" << std::endl;
    ...
    write_html_report(greedy_list, beam_list, out, w_state, w_total, op_penalty, slot_count, greedy_ms, beam_ms, TOPK_BEAM);
    write_all_results_json(greedy_list, beam_list, json_path_override.empty()?out:json_path_override);
    return 0;
}
#endif
